...
from Multi_Head_Diff_Transformer import EncoderOnlyTransformer
...
class ClozeTaskDataset(Dataset):
    def __init__(self, texts, tokenizer, max_length=128, mask_probability=0.15):
        ...
        self.mask_probability = mask_probability  # Probability of masking tokens
        self.inputs = []
        self.targets = []
        self.vocab_size = tokenizer.get_vocab_size()
        self.mask_token_id = tokenizer.token_to_id("[MASK]")  # Get the ID of the [MASK] token
        ...
        for text in texts:
            ...
            tokens = [min(t, self.vocab_size - 1) for t in tokens]  # Ensure tokens are within vocab size
            ...
            if len(tokens) > 8 and len(tokens) <= max_length:
                ...
                input_tokens = self._pad_sequence(masked_tokens)
                target_tokens = self._pad_sequence(labels)
    ...
    def _mask_tokens(self, tokens):
        """ Randomly mask tokens in the sequence for Cloze task (MLM). """
        labels = tokens.copy()  # The labels will be the original tokens that were masked
        for i in range(len(tokens)):
            if random.random() < self.mask_probability:
                tokens[i] = self.mask_token_id  # Mask the token
                labels[i] = tokens[i]  # The target is the masked token
            else:
                labels[i] = -100  # Ignore non-masked tokens during loss calculation (PyTorch convention)
...
def main():
    ...
    train_dataset = ClozeTaskDataset(dataset['train']['text'], tokenizer)
    val_dataset = ClozeTaskDataset(dataset['validation']['text'], tokenizer)
